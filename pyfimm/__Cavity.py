...
class Cavity(object):
    ...
    def calc(self, WLs, Display=False):
        ...
        The Cavity device will have the attributes `Cavity.S_ll` & `Cavity.S_rr` added, which are the left-to-left & right-to-right scattering matrices for the Right & Left devices, respectively (with reflections pointing at the central split).
        ...
        use pyFIMM.set_N(num_modes) to set the number of lateral modes to include in the calculation.
        '''
    ...
    def mode(self, num):
        ...
        Technically, this method returns a CavityMode object, so to find out what methods/attributes you can perform on `Cavity.mode(0)`, type `help(pyfimm.CavityMode)`'''
